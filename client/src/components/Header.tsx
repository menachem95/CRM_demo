  ...
  return (
    <AppBar position="static">
      <Toolbar sx={{ alignItems: "stretch" }}>
        ...
        >
          {["Home", "Customers", "Tasks", "Products", "Deals", "Meetings"].map(
            (text) => (
              <Button
                key={text}
                color="inherit"
                ...
                sx={{
                  height: "100%",
                  backgroundColor: isSelected(`/${text.toLowerCase()}`)
                    ? "rgba(255, 255, 255, 0.12)"
                    : "inherit", // שינוי צבע רקע בהתאם לכפתור שנבחר
                  "&:hover": {
        ...
        <Box sx={{ display: "flex" }}>
          <IconButton
            sx={{
              color: "inherit", // צבע דיפולטי
              "&:hover": {
                color: "secondary.main", // שינוי הצבע ב-hover
                backgroundColor: "#3E51B0"
              },
            }}
            onClick={() => navigate("/cart")}
          >
            <Badge badgeContent={cartItems.length} ><ShoppingCartIcon /></Badge>
